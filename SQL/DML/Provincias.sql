...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Verificar que el nuevo nombre no est� duplicado en otra provincia
        IF EXISTS (
            SELECT 1 
            FROM dbo.Provincias 
        ...
        BEGIN
            SET @CodigoError = 1;
            SET @Mensaje = 'El nombre de la provincia ya est� registrado en otra provincia.';
            RETURN;
        END;
...
BEGIN
    ...
    BEGIN TRY
        -- Retorna todas las provincias ordenadas alfab�ticamente
        SELECT 
            ProvinciaId,
            NombreProvincia
        FROM dbo.Provincias
...
BEGIN
    ...
    BEGIN TRY
        -- Retorna todos los distritos con el nombre del cant�n asociado
        SELECT 
            d.DistritoId AS DistritoId, 
            d.NombreDistrito,
            d.CantonId,
            c.NombreCanton               -- Nombre del cant�n (desnormalizaci�n)
